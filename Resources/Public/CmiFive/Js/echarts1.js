...
function getStatementsSuccess(verb, page, temp) {
  ...
  for (let i = 0; i < selection.length; i++) {
    ...
    registration = selection[i].context.registration;
    if (success) {
      if (registration === cmi5Controller.registration) successCounterMe++;
      else successCounter++;
    } else {
      if (registration === cmi5Controller.registration) failedCounterMe++;
      else failedCounter++;
    }
  }
  su.push({
    name: "Richtig",
    value: Math.round((successCounterMe * 100) / selection.length)
  });
  su.push({
    name: "Richtig Andere",
  ...
  su.push({
    name: "Falsch",
    value: Math.round((failedCounterMe * 100) / selection.length)
  });
  su.push({
    name: "Falsch Andere",
